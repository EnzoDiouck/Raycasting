...
double calcul_sphere(t_zpos vector, t_zpos cam, double rayon)
{
  double solution;
  double solution2;
  double delta;
  ...
  b = 2 * (cam.x * vector.x + cam.y * vector.y + cam.z * vector.z);
  c = pow(cam.x, 2) + pow(cam.y, 2) + pow(cam.z, 2) - pow(rayon, 2);
  delta = pow(b,2) - 4 * a * c;
